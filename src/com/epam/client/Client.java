package com.epam.client;

import java.net.Socket;
import java.net.UnknownHostException;
import java.io.*;

import javax.xml.parsers.ParserConfigurationException;
import javax.xml.transform.TransformerException;

import org.xml.sax.SAXException;

/**
 * 
 * @author Alexandr Kutashov
 * @version 1.2
 * 
 */

public class Client {

	private final String myName = "Sashok69";
	private String token = "a52fvccmarwyu2ol2occ22vilqmbkmml";
	private String address = "62.16.90.89";
	private int port = 10040;
	private Socket conn;
	protected Galaxy galaxy;
	private BufferedOutputStream out;
	private BufferedReader in;
	//private int num = 0;
	
	
	Client () {
		galaxy = new Galaxy(this);
	}
	
	/**
	 * @param args
	 * @throws IOException 
	 * @throws UnknownHostException 
	 * @throws SAXException 
	 * @throws ParserConfigurationException 
	 * @throws TransformerException 
	 * @throws InterruptedException 
	 */
	public static void main(String[] args) throws TransformerException, ParserConfigurationException, SAXException, IOException, InterruptedException {
		

		Client client = new Client();
		XMLData xml = new XMLData(client);
		
		
		while (true) {
			
			client.sendData(xml);
			xml.setData(client.getData());
			
			xml.parseData();
			xml.applyChanges(client.galaxy.makeMyMove());
		}
		

	}
	

	public void sendData (XMLData xml) throws UnknownHostException, IOException, TransformerException, ParserConfigurationException, SAXException, InterruptedException {

		conn = new Socket(address, port);
		out = new BufferedOutputStream(conn.getOutputStream());
		xml.sendData(out, true);
		
//		File file = new File("c:/out/message" + ++num + ".xml");
//		OutputStream fs = new FileOutputStream(file); 
//		xml.sendData(fs, true);
//		fs.flush();
	}
	
	public String getData () throws UnknownHostException, IOException {
		
		in = new BufferedReader(new InputStreamReader(conn.getInputStream()));
		StringBuilder msg = new StringBuilder();
		
		String str;
		while ((str = in.readLine()) != null) {
			msg.append(str);
		}
		
//		File file = new File("c:/in/message" + ++num + ".xml");
//		BufferedWriter fs = new BufferedWriter(new FileWriter(file));
//		fs.write(msg.toString());
//		fs.close();
		
		conn.close();
		
		return msg.toString();
	}

	public String getToken() {
		return token;
	}

	public void setToken(String token) {
		this.token = token;
	}

	public String getAddress() {
		return address;
	}

	public void setAddress(String address) {
		this.address = address;
	}

	public int getPort() {
		return port;
	}

	public void setPort(int port) {
		this.port = port;
	}

	public String getMyName() {
		return myName;
	}


	


	

}
